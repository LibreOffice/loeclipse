...
/**
 * Class creating the <code>services.rdb</code> file.
 * 
 * @author cedricbosdo
 *
 */
public class ServicesBuilder extends Job {
     ...
     * Constructor.
     * 
     * @param pProject the project to build.
     */
    public ServicesBuilder (IProject pProject) {
    ...
    protected IStatus run(IProgressMonitor pMonitor) {
        ...
        pMonitor.beginTask(Messages.getString("ServicesBuilder.BuildServiceTask"), 2); //$NON-NLS-1$
        ...
        if (unoProject != null) {
            status = syncRun(unoProject, pMonitor);
    ...
    /**
     * Synchronously run the service build.
     * 
     ...
     */
    public static Status syncRun(IUnoidlProject pUnoProject, IProgressMonitor pMonitor) {
        ...
        ILanguageBuilder langBuilder = pUnoProject.getLanguage().getLanguageBuidler();
        try {
            libraryPath = langBuilder.createLibrary(pUnoProject);
            
            pMonitor.worked(1);

            // Get the language specific arguments, eg LD_LIBRARY_PATH or CLASSPATH
            String[] env = langBuilder.getBuildEnv(pUnoProject); 
            ...
            String regcompCmd = regcomp + " -register " +  //$NON-NLS-1$
                "-r \"" + pUnoProject.getServicesPath().toOSString() + "\" " +  //$NON-NLS-1$ //$NON-NLS-2$
                "-br \"" + pUnoProject.getOOo().getTypesPath() + "\" " + //$NON-NLS-1$ //$NON-NLS-2$
                "-br \"" + pUnoProject.getOOo().getServicesPath() + "\" " + //$NON-NLS-1$ //$NON-NLS-2$
                "-c " + libpath; //$NON-NLS-1$

            // The normal messages of regcomp are sent to the err stream :(
            Process process = pUnoProject.getSdk().runToolWithEnv(pUnoProject, 
                    regcompCmd, env, pMonitor);
            ...
            if (!message.endsWith("succesful!\n")) { //$NON-NLS-1$
                ...
                IFile servicesFile = pUnoProject.getFile(pUnoProject.getServicesPath());
            ...
            UnoidlProjectHelper.refreshProject(pUnoProject, pMonitor);
        } catch (Exception e) {
            status = new Status(
                    IStatus.ERROR, OOEclipsePlugin.OOECLIPSE_PLUGIN_ID,
                    IStatus.ERROR, 
